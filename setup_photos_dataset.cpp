		...
		//for(int patch = 1; patch <= 3; patch++){
			...
			for (int l = 0; l < L; l++) {
				for(int y = 0; y < levels_hist1[l].cols; y++) {
					myfile << levels_hist1[l].at<float/*double*/>(0, y) << ((y + 1 < levels_hist1[l].cols)? "," : "");
				}
				//myfile << "\n";
...
void setup_photos_dataset::load_hists_for_each_level(vector<Mat> &levels_hist1/*, vector<Mat> &levels_hist2*/
	...
	int i = 0, cells_num = 0;//, num_bins = (int)(floor(pow(2.0, P)));
	//bool first = true;
	while (getline(hist_file, hist_values_line)) {
		...
		//if(first){
		levels_hist1[i] = Mat::zeros(1, /*8*/9 * cells_num, CV_32FC1/*CV_64FC1*/);
			while (pch != NULL){
				levels_hist1[i].at<float/*double*/>(0, col++) = atoi(pch);
				pch = strtok (NULL, ",");
			}
			//first = false;
		//} else {
